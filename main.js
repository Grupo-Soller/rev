...
<td class="text-center">${agency.totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</td>                        </tr>
                    `).join('')}
                </tbody>
            </table>
        </div>
...
// --- NOVO BLOCO: Função utilitária para formatação (deve ser inserida no main.js) ---
function daysToMonthsAndDaysText(totalDays) {
    // Usamos 30 dias como base para métricas de negócio (360/365)
    const daysInMonth = 30; 
    const months = Math.floor(totalDays / daysInMonth);
    const remainingDays = totalDays % daysInMonth;

    const monthPart = months > 0 ? (months === 1 ? '1 mês' : `${months} meses`) : '';
    const dayPart = remainingDays > 0 ? (remainingDays === 1 ? '1 dia' : `${remainingDays} dias`) : '';
...
// --- BLOCO DA FUNÇÃO renderAgingChart() (dentro do main.js) ---
function renderAgingChart() {
    const container = document.getElementById('agingChartContainer');
    if (!container) return;
    ...
    const agingData = {
        ...
        geral: {
            days: 119,
            label: 'Geral (à vista + parcelado)',
            color: 'var(--success-green-light)'
        },
        parcelado: {
            days: 178,
            label: 'Apenas parcelado',
            color: 'var(--warning-yellow)'
        }
    };
    
    // Calcula porcentagens de largura
    const geralPercent = (agingData.geral.days / agingData.maxDays) * 100;
    const parceladoPercent = (agingData.parcelado.days / agingData.maxDays) * 100;
    
    // Formata o texto por extenso
    const geralText = daysToMonthsAndDaysText(agingData.geral.days);
    const parceladoText = daysToMonthsAndDaysText(agingData.parcelado.days);

    const agingHTML = `
        <div class="aging-bar-wrapper" style="margin-bottom: 20px;">
            <div class="aging-label" style="color: var(--gray-700); font-weight: 600; margin-bottom: 5px;">
                ${agingData.geral.label} 
                <span style="float: right; color: ${agingData.geral.color};">
                    ${geralText} 
                </span>
            </div>
            <div class="progress-bar" style="height: 12px; border: 1px solid var(--gray-300);">
                <div class="progress-fill" style="width: ${geralPercent}%; background: ${agingData.geral.color};"></div>
            </div>
        </div>

        <div class="aging-bar-wrapper">
            <div class="aging-label" style="color: var(--gray-700); font-weight: 600; margin-bottom: 5px;">
                ${agingData.parcelado.label} 
                <span style="float: right; color: ${agingData.parcelado.color};">
                    ${parceladoText} 
                </span>
            </div>
            <div class="progress-bar" style="height: 12px; border: 1px solid var(--gray-300);">
                <div class="progress-fill" style="width: ${parceladoPercent}%; background: ${agingData.parcelado.color};"></div>
            </div>
        </div>
    `;
...
// --- FIM DA FUNÇÃO renderAgingChart() ---
...
// Adicionar após a função renderAgingChart()
function renderNFEmissionChart() {
    const container = document.getElementById('nfEmissionChartContainer');
    if (!container) return;
    ...
    const periodo1Percent = (nfData.periodos[0].quantidade / nfData.total) * 100;
    const periodo2Percent = (nfData.periodos[1].quantidade / nfData.total) * 100;
    
    const nfHTML = `

        <div class="nf-emission-bar-wrapper" style="margin-bottom: 20px;">
            <div class="nf-label" style="color: var(--gray-700); font-weight: 600; margin-bottom: 5px;">
                ${nfData.periodos[0].label}
                <span style="float: right; color: ${nfData.periodos[0].color};">
                    ${nfData.periodos[0].quantidade} de ${nfData.total} (${periodo1Percent.toFixed(0)}%)
                </span>
            </div>
            <div class="progress-bar" style="height: 20px; border: 1px solid var(--gray-300); border-radius: 4px; background: var(--gray-100);">
                <div class="progress-fill" style="width: ${periodo1Percent}%; background: ${nfData.periodos[0].color}; height: 100%; border-radius: 3px; transition: width 1s ease;">
                    <span style="color: white; font-weight: 600; display: block; text-align: center; line-height: 20px; font-size: 0.85rem;">
                        ${nfData.periodos[0].quantidade} NFs
                    </span>
                </div>
            </div>
        </div>

        <div class="nf-emission-bar-wrapper">
            <div class="nf-label" style="color: var(--gray-700); font-weight: 600; margin-bottom: 5px;">
                ${nfData.periodos[1].label}
                <span style="float: right; color: ${nfData.periodos[1].color};">
                    ${nfData.periodos[1].quantidade} de ${nfData.total} (${periodo2Percent.toFixed(0)}%)
                </span>
            </div>
            <div class="progress-bar" style="height: 20px; border: 1px solid var(--gray-300); border-radius: 4px; background: var(--gray-100);">
                <div class="progress-fill" style="width: ${periodo2Percent}%; background: ${nfData.periodos[1].color}; height: 100%; border-radius: 3px; transition: width 1s ease;">
                    <span style="color: white; font-weight: 600; display: block; text-align: center; line-height: 20px; font-size: 0.85rem;">
                        ${nfData.periodos[1].quantidade} NFs
                    </span>
                </div>
            </div>
        </div>
